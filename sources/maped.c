...
/*   Updated: 2016/10/23 10:36:19 by fjanoty          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		define_position(t_matrix ***map_mat, double *max, int i, t_env *e)
{
	...
	j = -1;
	if (!(rot_y = set_rotate(0, (e->dr1 * (e->anime_advence))
		+ ((i - ((max[0] - 1) / 2)) * e->phi1) / (max[0] - 1), 0)))
		return ;
	while (++j < max[1])
	{
		r = (300 * e->mult_z * e->map_d[j][i] / (max[2])) + (max[4] * e->k);
		vect_nb[0] = (max[3] * e->k) + r * cos(e->dr2 * e->anime_advence
			+ (e->phi2 * (j - ((max[1] - 1) / 2))) / (max[1] - 1));
		vect_nb[1] = r * sin((e->dr2 * (e->anime_advence))
		+ (e->phi2 * (j - ((max[1] - 1) / 2))) / (max[1] - 1));
